...
import { EVAL_WORKER_ACTION, MessageType, TMessage, WorkerErrorTypes } from "./types";
 ...
 */
function sendMessage(this: Worker | typeof globalThis, message: TMessage<unknown>) {
    this.postMessage(message);
}

async function responseHandler(worker: Worker, requestId: string): Promise<TPromiseResponse> {
...
const defaultErrorHandler: TransmissionErrorHandler = (messageId: string, timeTaken: number, responseData: unknown, e: unknown) => {
    console.error(e);
...
class Postoffice {
    ...
    async installLibrary(library: string) {
        const data = await this.messenger.request(EVAL_WORKER_ACTION.INSTALL_LIBRARY, {
            url: library,
            takenAccessors: [],
            takenNamesMap: {},
        });
    ...
    async uninstallLibrary(libraryDef: { accessor: string }) {
        await this.messenger.request(EVAL_WORKER_ACTION.UNINSTALL_LIBRARY, [libraryDef.accessor]);
    }
}
...
export class WorkerMessenger {
    ...
    async request(method: string, data = {}): Promise<any> {
        ...
        console.log("REQUEST ==>", { messageId, messageType: MessageType.REQUEST, body: { method, data } });
        sendMessage.call(this._worker, { messageId, messageType: MessageType.REQUEST, body: { method, data } });
     ...
     */
    static respond(messageId: string, data: unknown, timeTaken: number, onErrorHandler?: TransmissionErrorHandler) {
        try {
            sendMessage.call(self, {
                messageId,
                messageType: MessageType.RESPONSE,
