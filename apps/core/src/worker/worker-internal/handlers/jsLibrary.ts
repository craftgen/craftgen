...
import { createMessage, customJSLibraryMessages } from "../../constants";
import { invalidEntityIdentifiers } from "../../dependency-map/utils";
import { resetJSLibraries } from "../../js-library/reset-js-libraries";
import { makeTernDefs } from "../../js-library/tern-definition-generator";
import { JSLibraries, JSLibrary, libraryReservedIdentifiers } from "../../js-library";
...
export async function installLibrary(
    ...
    const libStore = takenAccessors.reduce((acc: Record<string, unknown>, a: string) => {
        acc[a] = self[a];
        return acc;
    }, {});
    ...
    try {
         ...
         */
        const unsetLibraryKeys = envKeysBeforeInstallation.filter((k) => self[k] === undefined);
        ...
        try {
            ...
            accessors.push(...difference(keysAfterInstallation, envKeysBeforeInstallation));
            ...
            for (let i = 0; i < accessors.length; i++) {
                if (takenNamesMap.hasOwnProperty(accessors[i]) || takenAccessors.includes(accessors[i])) {
                    const uniqueName = generateUniqueAccessor(accessors[i], takenAccessors, takenNamesMap);
            ...
            try {
                ...
                if (module && typeof module === "object") {
                    const uniqAccessor = generateUniqueAccessor(url, takenAccessors, takenNamesMap);
        ...
        try {
            ...
            throw new TernDefinitionError(`Failed to generate autocomplete definitions: ${name}`);
        }
...
export function uninstallLibrary(request: EvalWorkerRequest<Array<string>>) {
    const { data } = request;
    const accessor = data;
    try {
...
export async function loadLibraries(request: EvalWorkerRequest<JSLibrary[]>) {
    resetJSLibraries();
    ...
    try {
        for (const lib of libs) {
            ...
            try {
                 ...
                 */
                defaultAccessors.push(...Object.keys(libStore).filter((k) => libStore[k] !== self[k]));
 ...
 * @returns
 */
function generateUniqueAccessor(urlOrName: string, takenAccessors: Array<string>, takenNamesMap: Record<string, true>) {
    let name = urlOrName;
    ...
    if (!takenAccessors.includes(validVar) && !takenNamesMap.hasOwnProperty(validVar)) {
        return validVar;
    }
    let index = 0;
    /**
