...
function getScriptToEval(userScript: string, type: EvaluationScriptType): string {
...
function getUserScriptToEvaluate(userScript: string) {
    ...
    const script = getScriptToEval(unescapedJS, EvaluationScriptType.ASYNC_ANONYMOUS_FUNCTION);
    return { script };
}
...
function sanitizeScript(js: string) {
    ...
    return (self as any).evaluationVersion > 1 ? trimmedJS : unescapeJS(trimmedJS);
}
...
function resetWorkerGlobalScope() {
    for (const key of Object.keys(self)) {
        ...
        if (["evaluationVersion", "window", "document", "location"].includes(key)) continue;
