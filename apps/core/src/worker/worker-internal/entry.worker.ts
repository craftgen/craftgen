...
import { EVAL_WORKER_ACTION, EvalWorkerRequest, MessageType, TMessage, TransmissionErrorHandler } from "../types";

import evalExpression from "./handlers/evalExpression";
import { installLibrary, loadLibraries, uninstallLibrary } from "./handlers/jsLibrary";
...
const transmissionErrorHandlerMap: Partial<Record<EVAL_WORKER_ACTION, TransmissionErrorHandler>> = {
    //
};

async function asyncRequestMessageListener(e: MessageEvent<TMessage<EvalWorkerRequest>>) {
    ...
    WorkerMessenger.respond(messageId, data, end - start, transmissionErrorHandler);
}
