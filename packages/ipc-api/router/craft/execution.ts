...
export const craftExecutionRouter = createTRPCRouter({
    ...
    .mutation(async ({ ctx, input }) => {
      return await ctx.db.transaction(async (tx) => {
        const [executionNodeState] = await tx
          .insert(tenant.nodeExecutionData)
          .values({
            id: input.id,
          ...
          .onConflictDoUpdate({
            target: tenant.nodeExecutionData.id,
            set: {
              state: input.state as any,
              updatedAt: new Date(),
              ...(input.complete && { completedAt: new Date() }),
        ...
        await tx
          .update(tenant.workflowExecution)
          .set({
            updatedAt: new Date(),
          })
          .where(
            eq(
              tenant.workflowExecution.id,
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return await ctx.db
        ...
        .where(eq(tenant.workflowExecution.id, input.id))
        .returning();
    }),
  setEvent: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return await ctx.db
        .insert(tenant.workflowExecutionEvent)
        .values({
    ...
    .mutation(async ({ ctx, input }) => {
      return await ctx.db.transaction(async (tx) => {
        ...
        const [execution] = await tx
          .insert(tenant.workflowExecution)
          .values({
            workflowId: input.workflowId,
            workflowVersionId: input.workflowVersionId,
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return await ctx.db
        ...
        .where(eq(tenant.workflowExecution.id, input.executionId))
        .returning();
    }),
